...
begin
  ...
  select exists(
    ...
    where t.id = get_territory_statistics.territory_id
  ...
  from usage u
  where u.id = get_territory_statistics.territory_id;
  ...
  reassignment_periods as (
    select
      territory_id,
      greatest(extract(epoch from (next_given_at - picked_at)) / 86400.0, 0) as days
    from ordered_transactions
    where picked_at is not null
      and next_given_at is not null
  ),
  territory_stats as (
    select
      tr.territory_id,
  ...
  select
    coalesce(max(ts.assigned_percentage) filter (where ts.territory_id = get_territory_statistics.territory_id), 0),
    coalesce(avg(ts.assigned_percentage), 0),
    coalesce(max(ts.avg_reassignment_days) filter (where ts.territory_id = get_territory_statistics.territory_id), 0),
    coalesce(avg(ts.avg_reassignment_days), 0),
    coalesce(max(ts.avg_holding_days) filter (where ts.territory_id = get_territory_statistics.territory_id), 0),
    coalesce(avg(ts.avg_holding_days), 0),
    coalesce(max(ts.current_unassigned_days) filter (where ts.territory_id = get_territory_statistics.territory_id), 0),
    coalesce(max(ts.unique_persons) filter (where ts.territory_id = get_territory_statistics.territory_id), 0),
